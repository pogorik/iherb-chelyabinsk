...
// Расширенные → Импорт товаров → ссылка на прайс-лист (/api/vk-feed).

// Базовый URL магазина. Можно переопределить через env, иначе — боевой домен
// в ASCII/punycode-виде: VK (и другие импортёры YML) спотыкаются на
// кириллических IDN-адресах внутри фида, поэтому ссылки на товары строим
// на punycode-хосте (это тот же самый айхерб-74.рф).
export const VK_SHOP_URL = process.env.NEXT_PUBLIC_SITE_URL || "https://xn---74-5cdfx1a1d1b.xn--p1ai";
